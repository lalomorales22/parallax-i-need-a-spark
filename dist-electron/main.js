...
function resolveFiles(updateInfo, baseUrl, pathTransformer = (p) => p) {
  ...
  const result = files.map((fileInfo) => {
    ...
    return {
      url: (0, util_1$7.newUrlFromBase)(pathTransformer(fileInfo.url), baseUrl),
      info: fileInfo
    };
  });
  ...
  if (packageInfo != null) {
    result[0].packageInfo = {
      ...packageInfo,
      path: (0, util_1$7.newUrlFromBase)(pathTransformer(packageInfo.path), baseUrl).href
    };
  }
  return result;
}
...
class GenericProvider extends Provider_1$a.Provider {
  constructor(configuration, updater, runtimeOptions) {
    ...
    this.baseUrl = (0, util_1$6.newBaseUrl)(this.configuration.url);
  }
  get channel() {
  ...
  async getLatestVersion() {
    const channelFile = (0, util_1$6.getChannelFilename)(this.channel);
    const channelUrl = (0, util_1$6.newUrlFromBase)(channelFile, this.baseUrl, this.updater.isAddNoCacheQuery);
    for (let attemptNumber = 0; ; attemptNumber++) {
      try {
...
class BitbucketProvider extends Provider_1$9.Provider {
  constructor(configuration, updater, runtimeOptions) {
    ...
    this.baseUrl = (0, util_1$5.newBaseUrl)(`https://api.bitbucket.org/2.0/repositories/${owner}/${slug}/downloads`);
  }
  get channel() {
  ...
  async getLatestVersion() {
    ...
    const channelFile = (0, util_1$5.getChannelFilename)(this.getCustomChannelName(this.channel));
...
class BaseGitHubProvider extends Provider_1$8.Provider {
  constructor(options, defaultHost, runtimeOptions) {
    ...
    this.options = options;
    this.baseUrl = (0, util_1$4.newBaseUrl)((0, builder_util_runtime_1$b.githubUrl)(options, defaultHost));
    const apiHost = defaultHost === "github.com" ? "api.github.com" : defaultHost;
    this.baseApiUrl = (0, util_1$4.newBaseUrl)((0, builder_util_runtime_1$b.githubUrl)(options, apiHost));
  }
...
class GitHubProvider extends BaseGitHubProvider {
  ...
  async getLatestVersion() {
    ...
    const feedXml = await this.httpRequest((0, util_1$4.newUrlFromBase)(`${this.basePath}.atom`, this.baseUrl), {
...
${feedXml}`, "ERR_UPDATER_INVALID_RELEASE_FEED");
    ...
    const fetchData = async (channelName) => {
      channelFile = (0, util_1$4.getChannelFilename)(channelName);
      channelFileUrl = (0, util_1$4.newUrlFromBase)(this.getBaseDownloadPath(String(tag), channelFile), this.baseUrl);
  ...
  async getLatestTagName(cancellationToken) {
    ...
    const url = options.host == null || options.host === "github.com" ? (0, util_1$4.newUrlFromBase)(`${this.basePath}/latest`, this.baseUrl) : new url_1$3.URL(`${this.computeGithubBasePath(`/repos/${options.owner}/${options.repo}/releases`)}/latest`, this.baseApiUrl);
    try {
...
class KeygenProvider extends Provider_1$7.Provider {
  constructor(configuration, updater, runtimeOptions) {
    ...
    this.baseUrl = (0, util_1$3.newBaseUrl)(`https://${host}/v1/accounts/${this.configuration.account}/artifacts?product=${this.configuration.product}`);
  }
  get channel() {
...
class PrivateGitHubProvider extends GitHubProvider_1$1.BaseGitHubProvider {
  ...
  async getLatestVersion() {
    ...
    const channelFile = (0, util_1$2.getChannelFilename)(this.getDefaultChannelName());
  ...
  async getLatestVersionInfo(cancellationToken) {
    ...
    const url = (0, util_1$2.newUrlFromBase)(basePath, this.baseUrl);
    try {
...
class AppUpdater extends events_1$1.EventEmitter {
  ...
  async differentialDownloadInstaller(fileInfo, downloadUpdateOptions, installerPath, provider, oldInstallerFileName) {
    try {
      ...
      const blockmapFileUrls = (0, util_1$1.blockmapFiles)(fileInfo.url, this.app.version, downloadUpdateOptions.updateInfoAndProvider.info.version);
...
const child_process_1$4 = require$$1$5;
...
class AppImageUpdater extends BaseUpdater_1$4.BaseUpdater {
  ...
  doInstall(options) {
    ...
    if (options.isForceRunAfter) {
      ...
      (0, child_process_1$3.execFileSync)(destination, [], { env: env2 });
    }
    return true;
  }
}
...
if (typeof require$$1$4 === "string") {
  throw new TypeError("Not running in an Electron environment!");
}
const { env } = process;
const isEnvSet = "ELECTRON_IS_DEV" in env;
const getFromEnv = Number.parseInt(env.ELECTRON_IS_DEV, 10) === 1;
const isDev = isEnvSet ? getFromEnv : !require$$1$4.app.isPackaged;
const dbPath = isDev ? require$$1$1.join(require$$1$4.app.getAppPath(), "spark.db") : require$$1$1.join(require$$1$4.app.getPath("userData"), "spark.db");
...
function clearConversations(device_id) {
...
function setActiveModel(model_id) {
...
function saveNetworkStat(device_id, metric_name, metric_value) {
...
function getNetworkStats(device_id, metric_name, limit = 100) {
...
class PythonShell extends events_1.EventEmitter {
  ...
  constructor(scriptPath, options, stdoutSplitter = null, stderrSplitter = null) {
    super();
    function resolve(type2, val) {
      if (typeof val === "string") {
        return PythonShell[type2][val];
    ...
    options = extend2({}, PythonShell.defaultOptions, options);
    let pythonPath;
  ...
  parseError(data) {
    ...
    if (/^Traceback/.test(text)) {
      ...
      let exception2 = lines.pop();
      error2 = new PythonShellError(exception2);
      error2.traceback = data;
      error2.stack += os_1.EOL + "    ----- Python Traceback -----" + os_1.EOL + "  ";
      error2.stack += lines.slice(1).join(os_1.EOL + "  ");
    } else {
      error2 = new PythonShellError(text);
    }
    return error2;
  }
  /**
...
process.env.VITE_PUBLIC = require$$1$4.app.isPackaged ? process.env.DIST : path$m.join(process.env.DIST, "../public");
let win;
...
main$1.autoUpdater.on("error", (err) => {
  win == null ? void 0 : win.webContents.send("update-error", err);
});
main$1.autoUpdater.on("download-progress", (progress) => {
  win == null ? void 0 : win.webContents.send("update-download-progress", progress);
});
require$$1$4.app.whenReady().then(() => {
  ...
  require$$1$4.ipcMain.handle("start-host", (_event) => {
    console.log("Starting Host...");
    let scriptPath = path$m.join(__dirname, "../python_bridge/host.py");
  ...
  require$$1$4.ipcMain.handle("start-client", (_event) => {
    console.log("Starting Client...");
    let scriptPath = path$m.join(__dirname, "../python_bridge/client.py");
    if (require$$1$4.app.isPackaged) {
  ...
  require$$1$4.ipcMain.handle("stop-network-discovery", () => {
  ...
  require$$1$4.ipcMain.handle("get-all-devices", () => {
    return getAllDevices();
  });
  require$$1$4.ipcMain.handle("get-device", (_event, deviceId) => {
    return getDevice(deviceId);
  });
  let modelManagerProcess = null;
  require$$1$4.ipcMain.handle("browse-models", async (_event, task = "text-generation", limit = 20) => {
    return new Promise((resolve, reject) => {
      let scriptPath = path$m.join(__dirname, "../python_bridge/model_manager.py");
      if (require$$1$4.app.isPackaged) {
        scriptPath = path$m.join(process.resourcesPath, "python_bridge/model_manager.py");
      }
      const pythonPath = "python3";
      let output = "";
      const pyshell = new PythonShell_1(scriptPath, {
        mode: "text",
        pythonPath,
        pythonOptions: ["-u"],
  ...
  require$$1$4.ipcMain.handle("download-model", (_event, modelId) => {
    ...
    const pythonPath = "python3";
    modelManagerProcess = new PythonShell_1(scriptPath, {
      mode: "text",
      pythonPath,
      pythonOptions: ["-u"],
    ...
    modelManagerProcess.on("message", (message) => {
      console.log(message);
      win == null ? void 0 : win.webContents.send("model-download-progress", message);
    });
    modelManagerProcess.end((err) => {
      if (!err) {
        ...
        win == null ? void 0 : win.webContents.send("model-download-complete", modelId);
      }
    });
    return "Model download initiated";
  });
  require$$1$4.ipcMain.handle("get-local-models", () => {
    return getModels();
  });
  require$$1$4.ipcMain.handle("set-active-model", (_event, modelId) => {
    setActiveModel(modelId);
    return getActiveModel();
  });
  require$$1$4.ipcMain.handle("get-active-model", () => {
    return getActiveModel();
  });
  require$$1$4.ipcMain.handle("save-personality", (_event, personality) => {
    savePersonality(personality);
    return true;
  });
  require$$1$4.ipcMain.handle("get-personality", (_event, deviceId) => {
    return getPersonality(deviceId);
  });
  require$$1$4.ipcMain.handle("update-personality", (_event, id, updates) => {
    updatePersonality(id, updates);
    return true;
  });
  require$$1$4.ipcMain.handle("save-conversation", (_event, deviceId, role, content) => {
    saveConversation(deviceId, role, content);
    return true;
  });
  require$$1$4.ipcMain.handle("get-conversations", (_event, deviceId, limit) => {
    return getConversations(deviceId, limit);
  });
  require$$1$4.ipcMain.handle("clear-conversations", (_event, deviceId) => {
    clearConversations(deviceId);
    return true;
  });
  require$$1$4.ipcMain.handle("save-network-stat", (_event, deviceId, metricName, metricValue) => {
    saveNetworkStat(deviceId, metricName, metricValue);
    return true;
  });
  require$$1$4.ipcMain.handle("get-network-stats", (_event, deviceId, metricName, limit) => {
